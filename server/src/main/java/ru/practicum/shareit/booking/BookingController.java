...
public class BookingController {
    ...
    @PostMapping
    public ResponseEntity<BookingDtoOut> saveNewBooking(@RequestBody BookingDtoIn bookingDtoIn,
                                                        @RequestHeader(HEADER_USER_ID) Long userId) {
        log.info("Получен POST-запрос на бронирование. userId: {}, itemId: {}", userId, bookingDtoIn.getItemId());
        return ResponseEntity.ok(bookingService.saveNewBooking(bookingDtoIn, userId));
    }
    ...
    public ResponseEntity<BookingDtoOut> confirmBooking(@PathVariable Long bookingId,
        ...
        return ResponseEntity.ok(bookingService.confirmBooking(bookingId, userId, approve));
    }
